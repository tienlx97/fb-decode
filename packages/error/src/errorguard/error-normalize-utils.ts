...
import { getAll } from './error-x-fb-debug'
import { toReadableMessage } from './error-serializer'

import { NormalizeErrorProps, StackItemProps } from './types'
import { ErrorMetadata } from './error-metadata'
// import { TAALOpcodes, expect, getSimpleHash, performanceNow } from '@metamon/util'

import TAALOpcodes from '../utils/taal-opcodes'
import expect from '../utils/expect'
import { getSimpleHash } from '../utils/get-simple-hash'

import { performanceNow } from '@metamon/util/common/performance-now'
...
function normalizeError(error: Error2) {
  ...
  let lineNumber = error.lineNumber != undefined ? error.lineNumber : error.line
  let columnNumber =
  ...
  check && columnNumber == undefined && (columnNumber = stackItemList[0].column)
